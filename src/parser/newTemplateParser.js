...
export function findTemplates(text, templateName, title) {
  ...
  while (currIndex < text.length) {
    ...
    if (templateEndIndex === -1) {
      console.log(
        "Error: template end not found",
        title,
        text.substring(templateStartIndex, templateStartIndex + 100)
      );
      break;
    }
    const templateText = text.substring(
      templateStartIndex,
      templateEndIndex + 2
    );
    ...
    if (
      templateText.match(new RegExp(`^{{${escapeRegex(templateName)}\\s*[|}]`))
    ) {
...
export function getTemplateKeyValueData(templateText) {
  ...
  if (templateText) {
    ...
    while (!isTemplateEnd) {
      ...
      if (isTemplateEnd) {
        value = templateText
          .substring(equalSignIndex + 1, templateText.length - 2)
          .trim();
      } else {
        value = templateText
          .substring(equalSignIndex + 1, pipeSignIndex)
          .trim();
      }
