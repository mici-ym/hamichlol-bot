...
import { findTemplates } from './newTemplateParser.js';
import { getInnerLinks } from './wikiLinkParser.js';

export function getParagraphContent(articleText, paragraphName, title, withTitle = false) {
...
export function getAllParagraphs(articleText, articleTitle) {
  let currIndex = 0;
  ...
  while (currIndex !== -1) {
    ...
    if (articleText.substring(start, start + 3) === '===') {
      ...
      if (nextNewLine !== -1 && nextNewLine < end) {
        ...
        currIndex = end + 2;
        const content = getParagraphContent(articleText, title, articleTitle, true);
...
export function getUsersFromTagParagraph(articleContent, paragraphName) {
